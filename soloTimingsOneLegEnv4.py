...
class SoloTimingsOneLegEnv4:
    def __init__(self, config):
        ...
        self.num_actions = 4 # 8 describing new gait
    ...
    def reset(self):
        ...
        #self._gait_placeholder = np.stack((get_oscillator_function(0,8,16),
                                           #get_oscillator_function(8,16,16),
                                           #get_oscillator_function(0,8,16),
                                           #get_oscillator_function(8,16,16)))

        self._last_action = np.array([0.0, .0, .16, .16], dtype=np.float32)/max_timing
        self.store_actions(self._last_action * max_timing)
    ...
    def step(self, action):
        ...
        change  = (action[0].clip(-1,1) + 1)*0.5 > 0.5
        conf = (action[1:].clip(-1,1) + 1) * 0.5
        contact_config = ((conf * max_timing)/self.dt_mpc).astype(np.int)
