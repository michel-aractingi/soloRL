...
def get_oscillator_function(tnc, tc):
    assert tc >= tnc
    ...
    if tnc != 0:
        ...
        f = 2*tc
    v = np.ones((f,), dtype=np.float32)
    v[tnc:tc] = 0.0
    return np.concatenate([v]*500)

class SoloTimingsEnv:
    def __init__(self, config):
        ...
        self.num_actions = 8 # 8 describing new gait
    ...
    def reset(self):
        ...
        self._last_action = np.array([8,8, 0,8, 0,8, 8,8], dtype=np.int)
        self.store_actions((self._last_action * self.dt_mpc /  max_timing) )

        self._contacts = [get_oscillator_function(8,16),
                          get_oscillator_function(0,8),
                          get_oscillator_function(0,8),
                          get_oscillator_function(8,16)]
    ...
    def step(self, action):
        ...
        for l in range(0,len(contact_config), 2):
            if not np.array_equal(contact_config[l:l+2], self._last_action[l:l+2]):
                tnc, d = contact_config[l:l+2]
    ...
    def store_observation(self):
        ...
        if not self.flat_observation:
            observation = np.concatenate((observation, np.zeros((7,)))) 
